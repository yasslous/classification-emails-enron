...
Actions : Analyse exploratoire du dataset Enron, vérification de l'équilibre des classes (Spam vs Ham), et création de visualisations (comme les WordClouds) pour identifier les mots les plus fréquents utilisés par les spammeurs par rapport aux emails normaux.

2. 02_pretraitement_nlp.ipynb
Objectif : Préparer le texte brut pour la machine.

Actions : Implémentation des techniques de NLP. Les textes ont été convertis en minuscules, la ponctuation a été supprimée, puis nous avons appliqué la Tokenisation (découpage en mots) et la suppression des "Stop Words" (mots vides qui n'aident pas à la classification). Les données nettoyées ont été sauvegardées pour l'entraînement.
